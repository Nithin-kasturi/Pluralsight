        ...
        stage("Update image in deployment.yaml") {
            steps {
                script {
                    ...
                    echo "nithin" | sudo -S sed -i "s|image: nithin8/node-devops:.*|image: nithin8/node-devops:${IMAGE_TAG}|" /var/lib/jenkins/workspace/Node-devops/k8s/manifestsZ/deployment.yaml
                    '''
                }
            }
        }
